...
import { CreditCard, Clock, CheckCircle2, XCircle, AlertTriangle, ShieldAlert } from 'lucide-react';

export default function CheckoutPaymentModal({ order, onClose, onProcessPayment, isProcessing }) {
  const [timeLeft, setTimeLeft] = useState(300);
  const [idempotencyKey, setIdempotencyKey] = useState(`PAY-STORE-${Date.now()}`);
      ...
      <div className="glass-card" style={{ width: '100%', maxWidth: '500px', padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h2 style={{ fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <CreditCard size={22} color="#3b82f6" /> Checkout Payment Simulation
          </h2>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.85rem' }}>
            ...
            <span className="mono" style={{ fontWeight: 600, color: '#3b82f6' }}>{order.order_number}</span>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem' }}>
              ...
              <span>Stock Reservation Expiry:</span>
            </div>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            />
            <button className="btn btn-outline" onClick={() => setIdempotencyKey(`PAY-STORE-${Date.now()}`)}>
              New Key
            </button>
          </div>
        </div>
        ...
        <p style={{ fontSize: '0.85rem', fontWeight: 600, marginBottom: '10px', color: '#cbd5e1' }}>
          Test Gateway Payment Outcome:
        </p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <button 
            className="btn btn-primary" 
            style={{ width: '100%', padding: '12px', justifyContent: 'flex-start', background: '#10b981' }}
            disabled={isProcessing || timeLeft <= 0}
            onClick={() => onProcessPayment(order.id, 'SUCCESS', idempotencyKey)}
          >
            <CheckCircle2 size={18} /> Complete Payment (SUCCESS)
          </button>

          <button 
            className="btn btn-outline" 
            style={{ width: '100%', padding: '12px', justifyContent: 'flex-start', color: '#ef4444', borderColor: '#ef4444' }}
            disabled={isProcessing}
            onClick={() => onProcessPayment(order.id, 'FAILURE', idempotencyKey)}
          >
            <XCircle size={18} /> Simulate Payment FAILURE
          </button>
          ...
          >
            <AlertTriangle size={18} /> Simulate Payment TIMEOUT
          </button>

          <button 
            className="btn btn-outline" 
            style={{ width: '100%', padding: '10px', justifyContent: 'center', marginTop: '6px', fontSize: '0.8rem', color: '#3b82f6' }}
            disabled={isProcessing}
            onClick={() => {
          ...
          >
            <ShieldAlert size={16} /> Test Duplicate Payment Click (Prevent Double Charge)
          </button>
        </div>
      </div>
    </div>
